...
import { UpArrowSVG } from './icons/svgs';
...
function PostFloatingMenu(props: {
	...
	const handleFloatingBarDisplay = () => {
		...
		const isPostContentBottomInsideViewport =
			blogContent!.getBoundingClientRect().bottom + 40 <= currentViewportHeight;
	...
	return (
		<Tooltip.Provider delayDuration={200}>
			...
			<div className="post-floating-bar fixed left-0 right-0 z-50 flex h-12 w-full flex-wrap justify-center 2xl:h-14">
				<div className="border-1/2 relative mx-auto flex h-12 shrink flex-wrap items-center justify-center rounded-full border-slate-200 bg-white px-5 py-1 text-sm  text-slate-800 shadow-xl dark:border-slate-500 dark:bg-slate-700 dark:text-slate-50 2xl:h-14">
							...
							<div>
								...
								>
									<>
										...
										{post?.responseCount > 0 && (
											<span className="ml-0.5 pr-2">
												{kFormatter(post.responseCount + (post.replyCount || 0))}
											</span>
										)}
									</>
								</button>
